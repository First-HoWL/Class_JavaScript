...
function main(){
    ...
    a.textContent = (first(fArray, sArray));
    Div24.append(a);
...
function NextColor(){
    let div = document.querySelector(".DivWithCircle")

    active++;
    let activeElement = div.getElementsByClassName("circle")[active - 1]
    let unActiveElement = div.getElementsByClassName("circle")[active - 2]
    if (active == 4){
        active = 0
    }

    {
        if (active == 1) activeElement.classList.add("activeGreen")
        if (active == 2) activeElement.classList.add("activeYellow")
        if (active == 3) activeElement.classList.add("activeRed")

        if (active == 2) unActiveElement.classList.remove("activeGreen")
        if (active == 3) unActiveElement.classList.remove("activeYellow")
        if (active != 3) div.getElementsByClassName("circle")[2].classList.remove("activeRed")
